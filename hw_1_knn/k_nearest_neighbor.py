...
class KNearestNeighbor:
    ...
    def compute_distances_no_loops(self, X):
        ...
        # Euclidean distance formula: sqrt(sum((x_train-x_test)^2))
        # x_train = a
        # x_test = b
        ...
        dists = np.sqrt(
            # sum(a^2) + sum(b^2) - 2ab
            train_sum + test_sum - 2 * X@self.X_train.T
        )
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            frequency_array = np.bincount(closest_y)
            y_pred[i] = np.argmax(frequency_array)
